...
"""
    ...
    # Corrected predict method to only get predictions for assigned entities
    def improved_predict(self, text: str, tokens: List[str] = None, language: str = 'en') -> Dict:
        """
        Improved predict method that only requests predictions for assigned entities.
        Replace the predict method in MultiModelNER with this.
        """
        if tokens is None:
            tokens = text.split()
        ...
        # Iterate through model configs to get their assigned entities
        for model_name, config in self.model_configs.items():
        ...
        # Merge predictions (entity-specific merge)
...
def main():
    ...
    ner.save_predictions(de_results, 'predictions_german_multimodel_ver2.conll')
    metrics_de = ner.calculate_f1_scores(de_results, output_file='f1_scores_german_multimodel_ver2.txt')
    ...
    ner.save_predictions(en_results, 'predictions_english_multimodel_ver2.conll')
    metrics_en = ner.calculate_f1_scores(en_results, output_file='f1_scores_english_multimodel_ver2.txt')
