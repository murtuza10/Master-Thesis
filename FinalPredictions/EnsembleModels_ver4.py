...
class EntitySpecificMerger:
    ...
    Includes precedence system for conflict resolution.
    """
    ...
    def __init__(self, model_configs: Dict, model_precedence: List[str] = None):
        ...
        Args:
            ...
            model_precedence: List of model names in order of precedence (highest first)
                            If None, conflicts will use first-come-first-served
        """
        self.model_configs = model_configs
        self.model_precedence = model_precedence or []
        self._validate_no_overlaps()
        
        # Create precedence map for quick lookups
    ...
    def resolve_conflict(self, existing_model: str, new_model: str, 
                        position: int, token: str) -> str:
        """
        Resolve conflict between two models using precedence.
    ...
    def merge_predictions_entity_specific(
        ...
        Uses precedence system to resolve conflicts.
        ...
        for entity_type, predictions in all_predictions.items():
            ...
            for i, pred in enumerate(filtered_predictions):
                if pred != 'O':
                    # Check for conflicts
                    if final_labels[i] != 'O':
                        existing_entity, existing_model = filled_positions.get(i, ('unknown', 'unknown'))
                        ...
                        # Use precedence to resolve conflict
...
"""
    ...
    def improved_merge_predictions(self, all_predictions: Dict[str, List[str]], 
        ...
        """Merge predictions using precedence-based conflict resolution."""
        if not hasattr(self, '_entity_merger'):
            self._entity_merger = EntitySpecificMerger(self.model_configs, self.model_precedence)
...
def main():
    ...
    # Initialize multi-model NER with precedence
    ner = MultiModelNER(global_label_list, model_precedence=model_precedence)
    ...
    metrics_de = ner.calculate_f1_scores(de_results, output_file='f1_scores_german_multimodel_precedence.txt')
    ...
    metrics_en = ner.calculate_f1_scores(en_results, output_file='f1_scores_english_multimodel_precedence.txt')
