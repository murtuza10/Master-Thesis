...
from transformers import (
    ...
    DataCollatorForTokenClassification  # Add this import
)
...
def augment_with_mention_replacement(tokens, labels, replace_prob=0.2):
    """Fixed version that properly handles entity replacement"""
    new_toks, new_labels = [], []
    i = 0
    while i < len(tokens):
        ...
        if tag.startswith("B-") and random.random() < replace_prob:
            ...
            if entity_pools[ent_type]:
                ...
                # Create proper BIO labels for replacement
                if repl:
                    b_tag_idx = next(idx for idx, lbl in enumerate(label_list) if lbl == f"B-{ent_type}")
                    i_tag_idx = next(idx for idx, lbl in enumerate(label_list) if lbl == f"I-{ent_type}")
                    
                    new_toks.extend(repl)
                    new_labels.extend([b_tag_idx] + [i_tag_idx] * (len(repl) - 1))
                else:
                    # Fallback to original if replacement is empty
...
def augment_dataset_controlled(dataset, augmentation_ratio=0.5):
    """Create augmented dataset with proper structure"""
    augmented = []
    for _ in range(int(len(dataset) * augmentation_ratio)):
        ...
        if len(toks2) > 0 and len(tags2) > 0:
...
def objective(trial):
    ...
    training_args = TrainingArguments(
        output_dir=f"/lustre/scratch/data/s27mhusa_hpc-murtuza_master_thesis/agribert-english_specific-results-broad_3.0/hyperparameter_search_regularized/trial_{trial.number}",
        eval_strategy="epoch",
        save_strategy="epoch",
...
final_training_args = TrainingArguments(
    output_dir="/lustre/scratch/data/s27mhusa_hpc-murtuza_master_thesis/agribert-english_specific-results-broad_3.0-1/results/final_model_regularized_21",
    run_name="agribert-english_specific-regularized-training",
    eval_strategy="epoch",
    save_strategy="epoch",
    load_best_model_at_end=True,
    metric_for_best_model="f1",
...
# Use regular trainer with data collator
...
final_trainer.save_model("/lustre/scratch/data/s27mhusa_hpc-murtuza_master_thesis/agribert-english_specific_final_model_regularized_saved_broad_3.0-1")
tokenizer.save_pretrained("/lustre/scratch/data/s27mhusa_hpc-murtuza_master_thesis/agribert-english_specific_final_model_regularized_saved_broad_3.0-1")
...
print("Final model saved to ./results/agribert-english_specific_final_model_regularized_saved_broad_3.0-1")
print("="*80)