...
def objective(trial):
    dropout = trial.suggest_float("dropout", 0.05, 0.3)  # Range: 5% to 30%
    warmup_steps = trial.suggest_int("warmup_steps", 50, 500)  # Range: 50 to 500 steps
    def model_init_with_dropout():
        model = AutoModelForTokenClassification.from_pretrained(
            model_checkpoint,
            num_labels=len(label_list),
            hidden_dropout_prob=dropout,        # Apply dropout to hidden layers
            attention_probs_dropout_prob=dropout  # Apply dropout to attention
        )
        return model
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(tokenized_datasets['train_val'])):
        ...
        trainer = Trainer(
            model_init=model_init_with_dropout,
            args=training_args,
